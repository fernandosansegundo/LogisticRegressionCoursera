...
Though the above manual solution can be used for our purposes sometimes you may prefer a more complete and safer approach, using the function `epi.cp` provided by the `epiR` library (remember to install it if you haven't already).  The result is a list with two components, where the first one (called `cov.pattern`) is a data frame with information about the the covariate patterns. I'll just show you the first and last rows of the data frame (check the [help for the function](http://artax.karlin.mff.cuni.cz/r-help/library/epiR/html/epi.cp.html) if you want to know more details). 
...
And we can check thattheir sum equals the number of subjects with $y=1$ (recall $y$ is `LOW`):
...
Now the values of `LOW` for the observations in each decilic class are sumed to obtain the observed values of $y=1$. The `tapply` is the tool for this job, appplying an operation (the sum in this case) across the values of a factor.
...
We can use the above results to get this plot. I have labeled the points withthe number of the correspondng decilic class to make it easier to check which ones fall far from the diagonal line.  The 
...
To define the Hosmer - Lemeshow statistic we will use the first grouping startegy described in the lecture pdf. This means that we can directly use the $2\times J$ contingency table of observed and expected values for decilic classes that we constructed before. The Hosmer - Lemeshow statistic is:
...
If you [https://www.google.es/search?q=R+Hosmer+Lemeshow+test](google for "R Hosmer Lemeshow test") you will come across the `ResourceSelection` library. The library includes a `hoslem.test` function to perform the Hosmer - Lemeshow test. The result in our case is: 

```{r}
library(ResourceSelection)
(HL= hoslem.test(LOWBWT$LOW, fitted(glmLOWBWT), g = 9))
```
...
Note that the curve taht appears on page 31 of the lecture pdf is **not** the ROC curve for the `LOWBWT` logistic model in this lecture. 
